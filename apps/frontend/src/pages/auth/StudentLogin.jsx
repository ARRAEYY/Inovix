...
/**
 * StudentLogin — Google-first (spec §3.1: students must register with college email via Google).
 * Dev-login fallback for non-production.
 */
const StudentLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Route by role — students go to /student, others to their dashboard
  ...
  // Google Identity Services placeholder — wired when GOOGLE_CLIENT_ID is configured
  const handleGoogle = () => {
    ...
    <div className="flex min-h-screen w-full max-w-[1300px] mx-auto">
      <div className="flex-[1.3] px-8 pl-16 py-12 flex flex-col justify-center">
        ...
        <div className="mt-8">
          <h1 className="text-[5.5rem] font-extrabold leading-[1.05] mb-6 text-[#0F172A] tracking-tight">
            Good food.<br />
            <span className="text-[#EA580C]">Zero waiting.</span>
          </h1>
      ...
      <div className="flex-1 flex items-center justify-end px-8 pr-16 py-8">
        <div className="bg-white rounded-2xl border border-[#FCEAE1] p-12 w-full max-w-[440px]">
          <p className="text-xs font-bold tracking-widest text-[#94A3B8] uppercase mb-4">Student Login</p>
          <h2 className="text-[2rem] font-bold leading-tight mb-9 text-[#0F172A] tracking-tight">Welcome</h2>

          <form className="flex flex-col gap-5" onSubmit={handleLogin}>
            <div className="flex flex-col gap-2">
              <label htmlFor="email" className="text-sm font-semibold text-[#0F172A]">College email</label>
              <input type="email" id="email" placeholder="you@campus.edu" value={email}
