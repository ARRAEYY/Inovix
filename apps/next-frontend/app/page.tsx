...
function RootDispatcher() {
  ...
  if (isOutletRole(user.role)) return <OutletView />;
...
function LoginView() {
  ...
  function switchMode(next: AuthMode) {
    setMode(next);
    setError('');
    if (next === 'signin') selectRole(role);
  }
  ...
  const roleLabel = role === 'student' ? 'Student' : role === 'outlet' ? 'Outlet Admin' : 'Super Admin';
    ...
    <div className="login-container">
      <div className="login-left">
        ...
        <div className="hero-content">
          ...
          <p className="hero-description">
            Order from your favorite campus outlets and pick it up when it&apos;s ready.
            No queues. No waiting. No fuss.
          </p>
        </div>
      </div>
      ...
      <div className="login-right">
        <div className="login-card">
          <p className="card-subheading">{mode === 'signup' ? 'Student sign-up' : 'Dev login'}</p>
          <h2 className="card-title">{mode === 'signup' ? 'Create your account' : 'Welcome back'}</h2>
          ...
          {mode === 'signin' && (
            <div className="role-tabs" role="tablist">
              ...
              <button
                type="button"
                role="tab"
                className={`role-tab ${role === 'outlet' ? 'active' : ''}`}
                onClick={() => selectRole('outlet')}
              >
                <Store size={14} /> Outlet
              </button>
              <button
                type="button"
                role="tab"
          ...
          {mode === 'signup' ? (
            <form
              ...
              onSubmit={(e) => {
                e.preventDefault();
                // Real Google OAuth requires GOOGLE_CLIENT_ID on the backend.
                // Until that's wired up, surface a clear notice to the user.
                setError('Google Sign-In requires GOOGLE_CLIENT_ID to be configured. For dev testing, use the Student tab → dev-login.');
                toast.info('Use the Student tab → dev-login for now');
              }}
            >
              ...
              {error && (
                <div className="form-alert error-alert" role="alert" style={{ marginBottom: '0.85rem' }}>
                  <AlertCircle size={16} /> {error}
                </div>
              )}
              ...
              <div className="dev-mode-hint">
                <Info size={14} />
                <span>
                  Google Sign-In requires <code>GOOGLE_CLIENT_ID</code> to be configured on the
                  backend. For dev testing, switch to the <strong>Student</strong> tab and use
            ...
            <form className="login-form" onSubmit={handleSignIn}>
              ...
              {role === 'student' && (
                <p className="login-hint">
                  Students sign in with Google in production. Dev-login accepts the email above without a password.
                </p>
              )}
              ...
              <button type="submit" className="primary-btn" disabled={submitting}>
                {submitting ? (
                  ...
                  <>Sign in as {roleLabel} <span className="arrow">→</span></>
                )}
              </button>
              ...
              <p className="login-hint">
                Backend on port 3001 · all requests via <code>?XTransformPort=3001</code>
                <br />
                Dev accounts: <code>{DEV_ACCOUNTS.student.email}</code> ·{' '}
                <code>{DEV_ACCOUNTS.outletAdmin.email}</code> ·{' '}
                <code>{DEV_ACCOUNTS.superAdmin.email}</code>
              </p>
            </form>
          )}
        </div>
...
type AdminTab = 'overview' | 'users' | 'outlets' | 'orders' | 'refunds' | 'audit';
...
function AdminOutletManagement() {
  ...
  const statusMutation = useMutation({
    ...
    onMutate: async ({ id, status }) => {
      ...
      const prev = qc.getQueryData<AdminOutlet[]>(['nosh', 'admin', 'outlets']);
      if (prev) {
        qc.setQueryData<AdminOutlet[]>(
          ['nosh', 'admin', 'outlets'],
